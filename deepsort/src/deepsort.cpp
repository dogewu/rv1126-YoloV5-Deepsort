...
extern int idxInputImage;

extern condition_variable cv_videoRead;
extern mutex mtxQueueOutput;
extern mutex mtxQueueDetOut;

DeepSort::DeepSort(std::string modelPath, int batchSize, int featureDim, int cpu_id) {
    this->cpu_id = cpu_id;
...
void DeepSort::init() {
    ...
    featureExtractor1 = new FeatureTensor(enginePath.c_str(), cpu_id, 1, 1);
    featureExtractor1->init(imgShape, featureDim, NET_INPUTCHANNEL);

    // featureExtractor2 = new FeatureTensor(enginePath.c_str(), cpu_id, 1, 1);
...
void DeepSort::sort(cv::Mat& frame, DETECTIONSV2& detectionsv2) {
    ...
    if(true){
        ...
        // std::cout << "write frame1" << std::endl;
    ...
    if (flag1 && flag2) {
        double timebeforeKF = what_time_is_it_now();
        objTracker->predict();
        ...
        double timeafterKF = what_time_is_it_now();
        cout << "--------Time cost in KF and update: " << timeafterKF - timebeforeKF << "\n";
    }
