...
int Yolo::detect_process(){
	...
	while (1)
	{
		...
		if (input.index < this->det_interval || !(input.index % this->det_interval)) {
			...
			// cv::Mat img1;
			// img1 = cv::imread("origin_src.jpg");
			// cv::cvtColor(img1, img1, cv::COLOR_BGR2RGB);
			// cv::Mat img_dst;
			// cv::resize(img1, img_dst, cv::Size(640,640), 0, 0, 1);
			// cost_time = inference_yolo(img_dst);

			// cost_time = inference_yolo(input.img_pad);
			if(cost_time == -1)
