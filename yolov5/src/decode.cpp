                   ...
                   float threshold)
{
    ...
    // std::cout << "gridlen " << grid_len << std::endl;
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            for (int j = 0; j < grid_w; j++)
            {
                ...
                // std::cout << box_confidence << " " << threshold << std::endl;
                if (box_confidence >= threshold)
                {
                    ...
                    {
                        boxScores.push_back(maxClassProbs * box_confidence);
                        classId.push_back(maxClassId);
                        validCount++;
                        boxes.push_back(box_x);
                        boxes.push_back(box_y);
                 ...
                 detect_result_group_t *group)
{
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        float x1 = filterBoxes[n * 4 + 0] - w_offset;
        float y1 = filterBoxes[n * 4 + 1] - h_offset;
        float x2 = x1 + filterBoxes[n * 4 + 2];
        float y2 = y1 + filterBoxes[n * 4 + 3];
        int id = classId[n];
        // std::cout << w_offset << " " << h_offset << " " << resize_scale << " " << std::endl;
        ...
        // detbox.x2 = (int)((clamp(x2, 0, model_in_w) - w_offset) / resize_scale);
        // detbox.y2 = (int)((clamp(y2, 0, model_in_h)  - h_offset) / resize_scale);
        detbox.x1 = (int)(clamp(x1, 0, model_in_w) / resize_scale);
        detbox.y1 = (int)(clamp(y1, 0, model_in_h) / resize_scale);
        detbox.x2 = (int)(clamp(x2, 0, model_in_w) / resize_scale);
        detbox.y2 = (int)(clamp(y2, 0, model_in_h) / resize_scale);
        // std::cout << detbox.x1 << " " << detbox.y1 << " " << detbox.x2 << " " << detbox.y2 << std::endl;
        detbox.confidence = boxesScore[i];
        
        detbox.classID = id;
        char *label = labels[id];
        std::cout << labels[id] << " " << detbox.x1 << " " << detbox.y1 << " " << detbox.x2 << " " << detbox.y2 << std::endl;
        strncpy(detbox.name, label, OBJ_NAME_MAX_SIZE);
        group->results.push_back(detbox);
        // printf("result %d : (%d %d %d %d), %s\n", i, detbox.x1, detbox.y1,
        //        detbox.x2, detbox.y2, &labels[id]); 
